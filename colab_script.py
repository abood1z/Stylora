# ==================================================================================
# 🚀 STYLORA VIRTUAL TRY-ON UNIFIED GOOGLE COLAB SCRIPT
# ==================================================================================
# Instructions:
...
# 4. Replace the "NGROK_TOKEN" variable at the bottom with your own Ngrok Token.
# 5. Run the cell and wait for it to load. Copy the public Ngrok URL generated at the end.
# ==================================================================================
...
print("="*60)
print("🎬 [1/5] Setting up IDM-VTON Repository...")
print("="*60)

# 1. Clone IDM-VTON Repo if not present
...
print("\n" + "="*60)
print("📦 [2/5] Installing Packages (Resolving Dependency Conflicts)...")
print("="*60)

# 2. Install Dependencies (huggingface_hub compatibility is handled dynamically in tryon_api.py shim)
!pip install -q --upgrade huggingface_hub
!pip install -q accelerate==0.25.0 peft transformers==4.36.2 diffusers==0.25.0
!pip install -q einops fastapi uvicorn python-multipart pyngrok onnxruntime av
!pip install -q 'git+https://github.com/facebookresearch/detectron2.git'

print("\n" + "="*60)
print("📥 [3/5] Downloading Trained Checkpoints and Models...")
print("="*60)

# 3. Create checkpoints folders and download files
...
checkpoints = {
    "ckpt/densepose/model_final_162be9.pkl": "https://huggingface.co/yisol/IDM-VTON/resolve/main/ckpt/densepose/model_final_162be9.pkl",
    "ckpt/humanparsing/parsing_atr.onnx": "https://huggingface.co/yisol/IDM-VTON/resolve/main/ckpt/humanparsing/parsing_atr.onnx",
    "ckpt/humanparsing/parsing_lip.onnx": "https://huggingface.co/yisol/IDM-VTON/resolve/main/ckpt/humanparsing/parsing_lip.onnx",
    "ckpt/openpose/ckpts/body_pose_model.pth": "https://huggingface.co/yisol/IDM-VTON/resolve/main/ckpt/openpose/ckpts/body_pose_model.pth"
}
...
print("\n" + "="*60)
print("✍️ [4/5] Deploying tryon_api.py Server Script...")
print("="*60)

# 4. Write tryon_api.py directly using triple single-quotes (since tryon_api.py has no triple-quotes)
tryon_api_content = '''import os
import sys
import io
import json
...
# 1. التحديد التلقائي لبيئة العمل (كولاب أو محلي) لتعديل مجلد الكاش تلقائياً
...
if IS_COLAB:
    ...
    os.environ["HF_HOME"] = "C:/d/Stylora/huggingface_cache"
    print("💻 Running locally. Configured cache path to local Stylora folder")

# إضافة المسارات الحالية ومجلد gradio_demo للمسار لضمان عدم وجود أخطاء في الاستيراد
...
try:
    ...
    from detectron2.data.detection_utils import convert_PIL_to_numpy, _apply_exif_orientation as _apply_exif_orient

base_path = 'imaginairy/idm-vton-safetensors'
device = 'cuda:0' if torch.cuda.is_available() else 'cpu'

# 2. تحميل مشفرات النصوص والـ Tokenizers على المعالج الرئيسي (CPU) لتوفير الـ VRAM في كولاب
...
if 'cuda' in device:
    pipe.to(device)
    # الاحتفاظ بمشفرات النصوص على الـ CPU بعد نقل باقي الـ pipeline للـ GPU
    pipe.text_encoder.to('cpu')
    pipe.text_encoder_2.to('cpu')
    torch.cuda.empty_cache()
    gc.collect()

# مصفوفة التحويلات وتطبيع الصور
...
def blend_images(human_img, generated_img, mask, feather_radius=10):
    # دمج الصورة المولدة مع صورة الشخص الأصلية باستخدام قناع ناعم (Feathered Mask)
    # لحفظ تفاصيل الوجه، الخلفية، واليدين من التشوه.
...
def encode_prompt_optimized(pipe, prompt, negative_prompt, device):
    # استخراج ترميزات النص بنقل مشفرات النصوص مؤقتاً للـ GPU لتجنب OOM
    # ثم إعادتها فوراً للـ CPU وتنظيف الكاش.
...
def run_tryon_inference(
    ...
    mask, mask_gray = get_mask_location('hd', category_mapped, model_parse, keypoints)
    ...
    # 3. استخراج نصوص الوصف بذكاء
    print("⏳ Generating optimized text embeddings...")
    prompt = "model is wearing " + garment_des
    negative_prompt = "monochrome, lowres, bad anatomy, worst quality, low quality"
    
    prompt_embeds, negative_prompt_embeds, pooled_prompt_embeds, negative_pooled_prompt_embeds = encode_prompt_optimized(
        pipe, prompt, negative_prompt, device
    )
    
    prompt_cloth = "a photo of " + garment_des
    prompt_embeds_c = encode_cloth_prompt_optimized(
        pipe, prompt_cloth, negative_prompt, device
    )
    ...
    # تعريف دالة التغذية الراجعة للبث المباشر
    callback = None
        ...
        def callback_on_step_end(pipe_self, step, timestep, callback_kwargs):
            ...
            if step % 5 == 0:
                ...
                with torch.no_grad():
                    ...
                    # دمج ناعم للخطوة المؤقتة للحفاظ على الوجه
                    ...
                    # إرسال البيانات بشكل غير متزامن للعميل عبر الـ WebSocket
    ...
    # 4. دمج وحفظ معالم الشخص الأصلية بالكامل مع النتيجة النهائية
...
# السماح لجميع الأجهزة بالاتصال بالسيرفر المحلي (CORS)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def tryon_post(
    ...
    try:
        ...
        # تشغيل التوليد في خيط مستقل (Thread) لتجنب حظر السيرفر
        loop = asyncio.get_running_loop()
        final_img, _ = await loop.run_in_executor(
            None,
            lambda: run_tryon_inference(
                human_pil, garm_pil, category, description, steps, seed
            )
        )
        
        # تحويل النتيجة النهائية لبايتات JPEG وإرسالها
        byte_io = io.BytesIO()
...
async def tryon_ws(websocket: WebSocket):
    ...
    try:
        ...
        print(f"WS Config: category={category}, description={description}, steps={steps}, seed={seed}")
        
        # 2. استقبال صورة الشخص
        human_bytes = await websocket.receive_bytes()
        human_pil = Image.open(io.BytesIO(human_bytes)).convert("RGB")
        print("WS Human Image received.")
        
        # 3. استقبال صورة الملابس
        garm_bytes = await websocket.receive_bytes()
        garm_pil = Image.open(io.BytesIO(garm_bytes)).convert("RGB")
        print("WS Garment Image received.")
        
        # 4. تشغيل عملية التوليد والبث المباشر
        loop = asyncio.get_running_loop()
        final_img, _ = await loop.run_in_executor(
            None,
            lambda: run_tryon_inference(
                human_pil, garm_pil, category, description, steps, seed, websocket, loop
            )
        )
        
        # 5. إرسال الصورة النهائية مكتملة كـ Binary
        byte_io = io.BytesIO()
        final_img.save(byte_io, format="JPEG", quality=95)
...
with open("tryon_api.py", "w", encoding="utf-8") as f:
    f.write(tryon_api_content)
print("✅ tryon_api.py has been deployed successfully!")

print("\n" + "="*60)
print("🔌 [5/5] Connecting Ngrok Tunnel and Starting FastAPI Server...")
print("="*60)

# 5. Ngrok Authentication & Startup
# Replace this token with your own token from: https://dashboard.ngrok.com/get-started/your-authtoken
NGROK_TOKEN = "3Es2OiuZPYMc5OanCfu1OTWVMz8_6HASApu8dY9hFr8ViHTmy"

if not NGROK_TOKEN or "ضع" in NGROK_TOKEN:
    print("❌ ERROR: Please specify a valid NGROK_TOKEN before running!")
    sys.exit("Ngrok token missing.")

from pyngrok import ngrok
ngrok.kill()
ngrok.set_auth_token(NGROK_TOKEN)

# Start Ngrok Tunnel on Port 8001 (FastAPI port)
public_url = ngrok.connect(8001, bind_tls=True)
print("="*80)
print("🎉 SUCCESS! YOUR SERVER IS ONLINE AND CONNECTED!")
print("Use this URL in your Flutter App Settings (Settings Screen):")
print(f"🔗 {public_url}")
print("="*80)
